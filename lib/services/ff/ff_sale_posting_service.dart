...
class FFSalePostingService {
  ...
  Future<int> postSaleWithAllocations({
    ...
    final receivableAccountId = await _partyAccountService
        .ensureCustomerAccount(sale.customerId);

    final salesIncomeAccountId = await _systemAccounts.resolveAccountId(
      FFSystemAccountCodes.salesIncome,
    );

    final inventoryAccountId = await _systemAccounts.resolveAccountId(
      FFSystemAccountCodes.inventory,
    );

    final cogsAccountId = await _systemAccounts.resolveAccountId(
      FFSystemAccountCodes.costOfGoodsSold,
    );

    final db = await DatabaseHelper.instance.database;
    final saleCost = await _loadSaleCost(db, saleId);
  ...
  Future<int> postSaleWithExecutor(
    ...
    final receivableAccountId = await _partyAccountService
        .ensureCustomerAccountWithExecutor(db, sale.customerId);
    ...
    final cogsAccountId = await _resolveSystemAccountId(
      db,
      FFSystemAccountCodes.costOfGoodsSold,
    );
  ...
  Future<double> _loadSaleCost(DatabaseExecutor db, int saleId) async {
    final rows = await db.rawQuery(
      '''
      SELECT
        COALESCE(SUM(qty * purchase_price), 0) AS total
      FROM sale_items
      WHERE sale_id = ?
      ''',
      [saleId],
